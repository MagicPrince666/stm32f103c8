...
void LoraTransferTask(void const *argument);
...
int main(void)
{
    ...
    u2_printf("Lora Wan Test programe\r\n");

    /* Create the thread(s) */
    /* definition and creation of defaultTask */
    osThreadDef(defaultTask, LoraTransferTask, osPriorityNormal, 0, 256);
...
void LoraTransferTask(void const *argument)
{
    ...
    LED1 = 1;
    yl800tInit();
    LORA_EN = 0; // 低电平工作，高电平休眠
    LORA_SET = 1; // 低电平进入快速通道，中心节点模式有效

    for (;;) {
        LED0 = !LED0;
        LED1 = !LED1;
        // u2_printf("STM32 F103C8T6 %d\r\n", i++);
        osDelay(500);
    }
}
